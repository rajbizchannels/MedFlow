...
async function resetAndSeedDatabase() {
  console.log('\n🔄 Starting database reset (simple schema compatible)...\n');
  ...
  try {
    ...
    const sqlPath = path.join(__dirname, 'reset-and-seed-simple.sql');
    ...
    console.error('\nFull error:', error);
    console.error('\n💡 Tip: Run node scripts/inspect-schema.js to check your database schema');
    process.exit(1);
  } finally {
    if (client) {
      client.release();

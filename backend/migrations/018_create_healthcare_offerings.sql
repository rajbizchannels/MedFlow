...
CREATE TABLE IF NOT EXISTS package_offerings (
    id SERIAL PRIMARY KEY,
    package_id INTEGER REFERENCES offering_packages(id) ON DELETE CASCADE,
    offering_id INTEGER REFERENCES healthcare_offerings(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS patient_offering_enrollments (
    ...
    package_id INTEGER REFERENCES offering_packages(id) ON DELETE SET NULL,
    offering_id INTEGER REFERENCES healthcare_offerings(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS offering_promotions (
    ...
    offering_ids INTEGER[], -- Specific offerings
    package_ids INTEGER[], -- Specific packages
    category_ids INTEGER[], -- Specific categories
...
ALTER TABLE appointments ADD COLUMN IF NOT EXISTS offering_id INTEGER REFERENCES healthcare_offerings(id) ON DELETE SET NULL;
ALTER TABLE appointments ADD COLUMN IF NOT EXISTS package_enrollment_id INTEGER REFERENCES patient_offering_enrollments(id) ON DELETE SET NULL;

-- Create indexes for performance
CREATE INDEX idx_offerings_category ON healthcare_offerings(category_id);
CREATE INDEX idx_offerings_active ON healthcare_offerings(is_active);
CREATE INDEX idx_offerings_featured ON healthcare_offerings(is_featured);
CREATE INDEX idx_package_offerings_package ON package_offerings(package_id);
CREATE INDEX idx_package_offerings_offering ON package_offerings(offering_id);
CREATE INDEX idx_pricing_offering ON offering_pricing(offering_id);
CREATE INDEX idx_pricing_type ON offering_pricing(pricing_type);
CREATE INDEX idx_enrollments_patient ON patient_offering_enrollments(patient_id);
CREATE INDEX idx_enrollments_status ON patient_offering_enrollments(status);
CREATE INDEX idx_insurance_mappings_offering ON offering_insurance_mappings(offering_id);
CREATE INDEX idx_reviews_offering ON offering_reviews(offering_id);
CREATE INDEX idx_reviews_patient ON offering_reviews(patient_id);
CREATE INDEX idx_promotions_code ON offering_promotions(promo_code);
CREATE INDEX idx_promotions_active ON offering_promotions(is_active);
...
INSERT INTO healthcare_offerings (name, description, category_id, duration_minutes, cpt_codes, is_active, is_featured, base_price) VALUES
('Annual Physical Exam', 'Comprehensive annual health checkup including vitals, physical examination, and preventive care counseling', 1, 60, ARRAY['99385', '99386'], true, true, 250.00),
('Well-Child Visit', 'Pediatric wellness examination with developmental assessment and immunization review', 9, 45, ARRAY['99381', '99382'], true, true, 200.00),
('COVID-19 Vaccination', 'COVID-19 vaccine administration with observation period', 5, 30, ARRAY['91300', '0001A'], true, false, 0.00),
('Flu Shot', 'Seasonal influenza vaccination', 5, 15, ARRAY['90686', '90688'], true, true, 35.00),
('Diabetes Management Consultation', 'Comprehensive diabetes care visit with A1C review and treatment adjustment', 6, 45, ARRAY['99214', '99215'], true, false, 180.00),
('Mental Health Consultation', 'Initial psychiatric evaluation and treatment planning', 7, 60, ARRAY['90792'], true, true, 300.00),
('Blood Pressure Check', 'Quick blood pressure screening and assessment', 1, 15, ARRAY['99211'], true, false, 25.00),
('Telehealth Consultation', 'Virtual video consultation with healthcare provider', 10, 30, ARRAY['99213', '95'], true, true, 100.00);
...
(1, 'cash', 'Cash Price', 250.00, 250.00, true),
(1, 'insurance', 'Medicare', 250.00, 0.00, true),
(1, 'insurance', 'Blue Cross Blue Shield', 250.00, 25.00, true),
(2, 'cash', 'Cash Price', 200.00, 200.00, true),
(3, 'cash', 'Cash Price', 0.00, 0.00, true),
(4, 'cash', 'Cash Price', 35.00, 35.00, true),
(5, 'cash', 'Cash Price', 180.00, 180.00, true),
(6, 'cash', 'Cash Price', 300.00, 300.00, true),
(7, 'cash', 'Cash Price', 25.00, 25.00, true),
(8, 'cash', 'Cash Price', 100.00, 100.00, true);
...
('Annual Wellness Package', 'Complete annual wellness package including physical exam, labs, and vaccinations', 1, 'bundle', 365, 500.00, 15.00, 425.00,
ARRAY['Annual Physical Exam', 'Complete Blood Count', 'Lipid Panel', 'Flu Vaccination', '24/7 Nurse Hotline Access'], true, true),
('Diabetes Care Package', 'Comprehensive 6-month diabetes management program', 6, 'subscription', 180, 1200.00, 20.00, 960.00,
...
(1, 1, 1, 1), -- Annual Physical in Wellness Package
(1, 4, 1, 2), -- Flu Shot in Wellness Package
(2, 5, 3, 1); -- 3 Diabetes consultations in Diabetes Package
